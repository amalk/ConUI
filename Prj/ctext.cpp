...
void CText::draw(int fn)
{
    ...
    while(i < height() - 2)
    {
        if(_T[_loffset + i].strlen() > _offset)
        {
            ...
            {
                console.strdsp(&(((char*)_T[_loffset + i])[_offset]), absRow() + i + 1, absCol() + 1, width() - 2,
                               _curpos);
            }
            else
            {
                console.strdsp(&(((char*)_T[_loffset + i])[_offset]), absRow() + i + 1, absCol() + 1,
                               _T[_loffset + i].strlen(), _curpos);
            }
        }
